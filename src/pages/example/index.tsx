...
const Example = () => {
  ...
  const [list, setList] = useState<{ result: any[]; loading: boolean }>({
    result: [],
    loading: true,
  });
    ...
    <>
      <AppBox p={3}>
        <Stack spacing={3}>
          ...
          <Box display={"flex"}>
            ...
            <Button variant={"outlined"} component={"label"}>
              <input
                ...
                accept=".xlsx, .xls, .csv"
                onChange={(event) =>
                  setFileUpload(event.target.files && event.target.files[0])
                }
              />
              Upload
            </Button>
          ...
          <Divider>Form initial by number</Divider>
          <Box
            sx={{
              display: "flex",
              flexDirection: "row",
          ...
          >
            <AppInput
              value={numConfig}
              onChange={(e) => setNumConfig(e.target.value)}
            />
            <Button
              sx={{
                textAlign: "center",
              ...
              onClick={() => handleAddConfig(numConfig)}
            >
              ...
              objList.map((item, index) => {
                ...
                return (
                  <Box key={index}>
                    <AppInput
                      value={name}
                      onChange={(e) => handleSetForm(index, e)}
                    />
                  </Box>
                );
              })}
